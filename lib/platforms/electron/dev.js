...
const { decideby, extend } = require('ts-fns')
...
module.exports = function dev(source) {
  ...
  plugins.push(new AfterHookPlugin(() => {
    ...
    const { name: pkgName, version: pkgVer } = readJSON(path.resolve(__dirname, '../../package.json'))
    const { name = pkgName, version = pkgVer, icon, build = {} } = readJSON(electronFile)
    ...
    if (exists(path.resolve(distDir, 'node_modules'))) {
      remove(path.resolve(distDir, 'node_modules'))
    }
    symlink(path.resolve(cwd, 'node_modules'), path.resolve(distDir, 'node_modules'))
  ...
  // support ts
  if (configs.typescript) {
    mainConfig.module = { rules: [tsxLoaders] }
  }
  ...
  server.listen(port, host, () => console.log(chalk.bold.blue(`DevServer has been setup at http://localhost:${port}.`)))
}
