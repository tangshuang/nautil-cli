...
const { decideby, map, parse, find } = require('ts-fns')
...
module.exports = function(platform, configs, source) {
  ...
  const { source: sources = {} } = configs
  const item = find(sources, (_, key) = key === source)
  if (item) {
    const { extensions: exts } = item
    if (exts) {
      extensions.unshift(...exts)
    }
  }
  ...
  const webpackConfig = {
    ...
    devServer: {
      ...
      contentBase: path.resolve(cwd, 'dist', source),
      index: path.resolve(cwd, 'dist', source, 'index.html'),
  ...
  if (configs.source && configs.source[source] && configs.source[source].library) {
    ...
    webpackConfig.output.globalObject = `typeof globalThis !== 'undefined' ? globalThis : typeof window !== 'undefined' ? window : typeof global !== 'undefined' ? global : typeof self !== 'undefined' ? self : this`
  }
