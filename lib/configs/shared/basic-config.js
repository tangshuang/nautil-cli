...
module.exports = function(platform, configs) {
  ...
  return {
    ...
    devtool: env === 'production' ? undefined
      : env === 'development' ? (platform === 'dom' && configs.hot ? '#eval-source-map' : 'source-map')
      : 'source-map',
    devServer: {
    ...
    plugins: [
      ...
      new ModuleReplacePlugin(
        source => source.indexOf(path.resolve(cwd, 'node_modules/nautil/lib/elements')) === 0,
        source => decideby(() => {
          if (process.env.NODE_ENV === 'development') {
            return source
          }
          if (platform === 'native') {
            return source.replace('nautil/lib/elements', 'nautil/native/elements')
          }
          if (['dom', 'web-component', 'ssr'].includes(platform)) {
            return source.replace('nautil/lib/elements', 'nautil/dom/elements')
          }
          // TODO: miniprogram
          return source
        })
