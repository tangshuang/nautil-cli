...
WebComponentCssPlugin.prototype.apply = function(compiler) {
  const { options } = this
  const { placeholder = 'WEB_COMPONENT_CSS_TEXT' } = options
  compiler.hooks.emit.tap('WebComponentCssPlugin', function(compilation) {
    ...
    chunks.forEach((chunk) => {
      const { files } = chunk
      const newFiles = Array.from(files).filter(file => file.split('.').pop() !== 'css')
      chunk.files = newFiles
    })
  })
  compiler.hooks.compilation.tap('HtmlSkipAssetsWebpackPlugin', (compilation) => {
    HtmlPlugin.getHooks(compilation).alterAssetTags.tapAsync(
      'HtmlSkipAssetsWebpackPlugin',
      (data, cb) => {
        // remove all css files to inject into html
