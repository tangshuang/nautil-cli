    ...
    compiler.hooks.compilation.tap('ModuleModifyPlugin', (compilation) => {
      compilation.hooks.succeedModule.tap('ModuleModifyPlugin', (module) => {
        ...
        const replaceBy = (source, name) => {
          ...
          source.replace(0, content.length, replacement, name)
        ...
        if (originalSource instanceof SourceMapSource) {
          const { _name, _sourceMapAsString, _originalSourceAsString, _innerSourceMapAsString, _removeOriginalSource } = originalSource
          const source = new ReplaceSource(originalSource, _name)
          const newContent = replaceBy(source, _name)
          const newSource = new SourceMapSource(newContent, _name, _sourceMapAsString, _originalSourceAsString, _innerSourceMapAsString, _removeOriginalSource)
