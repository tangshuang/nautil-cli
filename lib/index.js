...
commander
  ...
  .option('--global, -g [global]', 'install dependencies into global installed nautil-cli (not current project).')
  .option('--force, -f [force]', 'force reinstall dependencies from nautil-cli provided, pkgs... should not be given.')
  .action(function(pkgs, options) {
    const { init, lazy } = readJSON(path.resolve(__dirname, 'package.json'))
