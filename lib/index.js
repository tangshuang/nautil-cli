      ...
      return inquirer.prompt([
        ...
        },
        {
          ...
          when(answers) {
            ...
            return platforms.includes('native')
          },
        },
        ...
        if (platforms.includes('native')) {
          ...
          const { dependencies } = collectBy(['native'])
    ...
    const pushDeps = (items, platform, dev) => {
      ...
      lazy.forEach((info) => {
        if (isOneInArray(info.platforms, [platform]) && info[depKey]) {
          const deps = Object.entries(info[depKey]).map(([key, value]) => `${key}@${value}`)
          items.push(...deps)
        }
      })
    }
    ...
    pkgs.forEach((pkg) => {
      if (pkg[0] === '+') {
        ...
        if (supports[platform]) {
          ...
          const deps = Object.entries(peerDependencies).map(([key, value]) => `${key}@${value}`)
          items.push(...deps)
        }
        else {
...
commander
  .command('run <source> <appos>')
  .description('Serve up react native app.')
  .action(function(source, appos) {
    if (!['ios', 'android'].includes(appos)) {
      console.error(chalk.red('appos should only be choosed from ios|android'))
      shell.exit(1)
    }
    ...
    const appConfigFile = path.resolve(srcDir, 'config.json')
    if (!exists(appConfigFile)) {
      console.error(chalk.red(`${appConfigFile} is not existing!`))
      shell.exit(1)
    }

    const { name: AppName } = readJSON(appConfigFile)
    ...
    shell.cd(appDir)
    shell.exec(`npx react-native run-${appos}`) // react-native run will end after building, we do not need to run it in async task
