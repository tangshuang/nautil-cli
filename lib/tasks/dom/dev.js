...
const { jsxLoaders, babelConfig, loadAsRequest } = require('../../configs/rules/jsx')
...
module.exports = function dev() {
  ...
  const customConfig = {
    ...
    devServer: {
      ...
      before(app) {
        if (configs.dll) {
          app.get('/react.dll.js', (req, res) => res.sendFile(path.resolve(distDir, 'react.dll.js')))
          app.get('/nautil.dll.js', (req, res) => res.sendFile(path.resolve(distDir, 'nautil.dll.js')))
        }
      },
    },
    devtool: configs.hot ? '#eval-source-map' : 'eval',
  }
  ...
  const { dev: modifyDevConfig, dll: modifyDllConfig } = decideby(() => {
  ...
  const serve = () => {
    ...
    server.listen(config.devServer.port, () => console.log(chalk.bold.blue(`DevServer has been setup at http://localhost:${port}`)))
  }
  ...
  if (
    ...
    let dllConfig = dll(manifestDir, distDir)
    if (modifyDllConfig) {
